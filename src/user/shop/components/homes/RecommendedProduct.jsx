...
const RecommendedProduct = () => {
  ...
  useEffect(() => {
    const fetchRecommendedProducts = async () => {
      try {
        const data = await productService.getRecomendedProduct();
        setListRecommendedProducts(data);
        setLoading(false);
      } catch (err) {
  ...
  const currentItems = listRecommendedProducts.slice(
  ...
  return (
    <section className="font-poppins p-3 space-y-3">
      <p className="font-bold text-md text-orange-900 md:text-xl">
        RECOMMENDED PRODUCT
      </p>
