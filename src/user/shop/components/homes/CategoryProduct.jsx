...
const CategoryProduct = () => {
  ...
  const categories = [
    ...
    { name: "Harley Davidson", bgImage: "./shop/homes/category-product/harleydavidson.svg" },
  ];
  ...
  return (
    <section className="bg-orange-100 font-poppins p-3 space-y-3">
      ...
      <div className="relative">
          ...
          {currentCategories.map((category, index) => (
            ...
            >
              ...
              <div className="h-16 bg-white flex items-center justify-center overflow-hidden relative rounded-full shadow-sm w-16">
                <img src={category.bgImage} alt={category.bgImage} className="w-20 h-20 transform group-hover:scale-110 transition duration-300" />
              </div>

              {/* Nama Kategori */}
              <p className="italic font-serif text-orange-900 group-hover:text-orange-700">{category.name}</p>
            </div>
          ))}
        </div>
        ...
        <div>
          {categoryPage > 0 && (
            <button onClick={() => setCategoryPage((prev) => prev - 1)} className="absolute duration-300 left-0 rounded-full text-orange-500 top-1/2 transform -translate-y-1/2 hover:text-orange-700">
          ...
          {categoryPage < totalPages - 1 && (
            <button onClick={() => setCategoryPage((prev) => prev + 1)} className="absolute duration-300 right-0 rounded-full text-orange-500 top-1/2 transform -translate-y-1/2 hover:text-orange-700">
