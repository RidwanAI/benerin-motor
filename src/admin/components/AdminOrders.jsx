...
const AdminOrders = () => {
  ...
  */
  ...
  const updateOrderStatus = async () => {
    ...
    try {
      ...
      setOrders((prevOrders) => prevOrders.map((order) => (order.id === selectedOrder.id ? { ...order, status } : order)));
      closeModal();
  ...
  */
  const filteredOrders = orders.filter(
    (order) => order.user?.name.toLowerCase().includes(searchQuery.toLowerCase()) || order.status.toLowerCase().includes(searchQuery.toLowerCase()) || order.shippingAddress.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="flex flex-col font-poppins min-h-screen">
      ...
      <div className="flex-1 bg-slate-100 overflow-auto">
              ...
              <tbody>
                {currentOrders.map((order) => (
                  <tr key={order.id} className="border-t text-sm">
                    ...
                    <td className="px-4 py-3">{`Rp.${parseFloat(order.totalPrice).toLocaleString("id-ID", { minimumFractionDigits: 2 })}`}</td>
                    <td className="px-4 py-3">{order.status}</td>
                    <td className="px-4 py-3">
                      {order.paymentProof ? (
                        <a href={order.paymentProof} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
                          View
                        </a>
                      ) : (
                        "None"
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => openModal(order)} className="bg-yellow-500 px-3 py-1.5 text-white rounded hover:bg-yellow-600">
                        Edit Status
                      </button>
                    </td>
                  </tr>
        ...
        {/* Fitur -> Pop Up Edit Status Order */}
          ...
          <div className="bg-slate-500 bg-opacity-50 fixed inset-0 flex items-center justify-center px-3 z-50">
            <div className="bg-white p-3 rounded-sm space-y-3 w-full md:w-1/3">
              ...
              <div className="flex justify-end gap-3">
                <button onClick={closeModal} className="bg-gray-300 px-4 py-2 rounded hover:bg-gray-500">
                  Cancel
                </button>
                <button onClick={updateOrderStatus} className="bg-orange-500 px-4 py-2 text-white rounded hover:bg-orange-700">
                  Save
                </button>
              </div>
            </div>
